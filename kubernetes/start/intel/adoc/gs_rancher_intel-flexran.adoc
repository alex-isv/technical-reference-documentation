...
// -
:title: Intel® FlexRAN™ Reference Solution
:subtitle: Deploy Cloud Native Open RAN with Intel and SUSE
//:subtitle: FlexRAN™ Deployment Guide for Rancher Kubernetes Cluster
:productname: Rancher 2.6
//  SLES 15 SP3 RT, SLE Micro 5.2 RT, RANCHER 2.6, RKE2
:platform1: Rancher 2.6 by SUSE
:platform2: SUSE Linux Enterprise Server 15.3 Real Time
:platform2short: SLES 15 RT
:platform3: SUSE Linux Enterprise Micro 5.2 Real Time
:platform3short: SLE Micro 5.2 RT
:otherproduct1: Intel® FlexRAN™ 22.07
:otherproduct1short: Intel FlexRAN
:k8s1: Rancher Kubernetes Engine 2
:k8s1short: RKE2
:k8s2short: K3s
:usecase: SUSE/Intel Open RAN Implementation
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// Can we use an approved logo lock-up here?


=== Motivation

// Update with:
// First paragraph/sentence: What is Intel FlexRAN?
// Second paragraph/sentence: Why should anyone care?
This document provides detailed guidance for deploying a cloud native, Open RAN stack with Intel® FlexRAN™ on SUSE Rancher Kubernetes cluster.
This demonstration shows required steps to install and configure Intel's FlexRAN PHY Reference Design using SUSE Linux Enterprise Server Real Time as the base Operating System and SUSE Linux Enterprise Micro Real Time as target hosts for Rancher RKE2 or K3s Kubernetes cluster with SUSE Rancher Server to manage this deployment.
 
The {otherproduct1}&#8482; is a 4G and 5G baseband PHY Reference design utilizing Xeon series processors and consists of a L1 library for SkyLake/CascadeLake platforms to demonstrate the capabilities of the software running 5GNR L1 features. See  https://www.intel.com/content/www/us/en/developer/topic-technology/edge-5g/tools/flexran.html [FlexRAN™ Reference Architecture] for more information.
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
This guide details deployment and configuration of {otherproduct1} PHY reference design with {platform1}, {platform2}, and {platform3}.


=== Audience

// Who is this guide for and what skills are needed?
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
The goal is to empower Telco developers and/or engineers to put together an Intel® FlexRAN™ test and/or proof-of-concept (PoC) environment leveraging the benefits of the complete SUSE stack from the real-time OS to Kubernetes orchestration and management.
...
In this section, you review some of the prerequisites you will need to successfully deploy this implementation.


=== Hardware configuration


Servers used for the development of this guide have the following specifications:
...
[NOTE]
====
For more details on required components, see _Installation Guide Software Release v22.07 (Doc. No.: 575834-15.0)_ and _FlexRAN™ 5GNR Reference Solution 22.07 - PHY Software Documentation (Doc. No.:603577)_.
====
...
Among the most important are CPU C state and P state.
...
For CPU power management in the bios settings use OS DBPM (or similar) setting to control setting from OS to manipulate processor frequencies.
...
[NOTE]
====
For more details, please review section 2.4.2 of _FlexRAN Software Reference Solution Cloud-Native Setup (Intel® Doc. No. 575834-15.0)_ and _BIOS Settings for FlexRAN Platforms Based on Intel® Xeon® Processors (Doc. No.: 675g685, Rev.: 1.3)_.
====


=== OS requirements

Intel® FlexRAN™ stipulates a real-time kernel, as listed in Intel® FlexRAN™  Installation Guide (Doc. No.: 575834-15.0)
...
[NOTE]
====
 During the installation, add a sufficient space to the /opt or /var directory which will be used for most Intel components and containers. We would recommend 200 Gb of storage for these directories. Don’t install FlexRAN under root directory.
====
...
[NOTE]
====
 Settings depends on the number of CPU and isolated cores.
...
In this test Xeon 4316 was used with 2.6 GHz.
...
* **The 2nd method to change AVX512 frequency is to install Intel msr-tools with the following commands:**
...
[NOTE]
====
 Values in the script are taking from Intel document #637779 (for ice lake family)  specific to your CPU avx512 numbers. (2.6 GHz in the above example)
====
...
[NOTE]
====
 Make sure that the installation directory has enough space. Intel® oneAPI is using about 40Gb of space.
====
...
To demonstrate a simple functionality:
...
“FlexRAN is Intel 4G and 5G baseband PHY Reference Design, which uses Xeon® series Processor with Intel Architecture. This 5GNR Reference PHY consists of a L1 binary which are validated on an Intel® Xeon® SkyLake / CascadeLake platforms and demonstrates the capabilities of the software running different 5GNR L1 features”. - https://github.com/intel/FlexRAN 

SUSE provides all the elements for an open-source, enterprise-grade, software-defined stack for cloud-native orchestration and management.  Ranging from our SUSE Linux Enterprise OS family to Kubernetes distribution and consistent cluster operations, security/authentication/policy management/governance and development platform services via our Rancher Management Platform.

This Intel® FlexRAN™ Solution Setup provides detailed steps to build, test, and deploy a cloud-native implementation of FlexRAN™ 22.07 with a complete SUSE software-defined, OS and Kubernetes stack.

SUSE Linux Enterprise 15 SP3 Real-Time (SLES 15 SP3 RT) as the OS platform to build the FlexRAN™ containers. Deployment and management of the containers leverages SUSE Linux Enterprise Micro 5.2 Real Time (SLE Micro RT) , Rancher Kubernetes Engine v2 (RKE2), and Rancher Management.
